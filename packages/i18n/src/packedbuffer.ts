/*
 * packedbuffer.ts - represent a packed buffer of bytes
 *
...
class PackedBuffer {
	...
	/**
	 * @param {ArrayLike.<number>} buffer the binary buffer of unsigned bytes
	 */
	 ...
	 */
	getLongs (num: number): number[] | undefined {
		let result;
	 ...
	 */
	getBytes (num: number): number[] | undefined {
		let result;
		if (this.buffer && this.index < this.buffer.length) {
			result = [];
	 ...
	 */
	getUnsignedBytes (num: number): number[] | undefined {
		let result;
		if (this.buffer && this.index < this.buffer.length) {
			result = [];
			for (let i = 0; i < num && this.index < this.buffer.length; i++) {
