...
describe('SpotlightContainerDecorator', () => {
	...
	test('should set active container to parent container on mouse leave', async () => {
		...
		await user.pointer({target: component.parentElement!, coords: unhoverPosition});
	...
	test('should forward onFocusCapture events', () => {
		const spy = jest.fn();
		let focus: ((ev: Record<string, unknown>) => void) | undefined;

		const Component = SpotlightContainerDecorator(({onFocusCapture}: {onFocusCapture?: (ev: Record<string, unknown>) => void}) => {
	...
	test('should forward onBlurCapture events', () => {
		const spy = jest.fn();
		let blur: ((ev: Record<string, unknown>) => void) | undefined;

		const Component = SpotlightContainerDecorator(({onBlurCapture}: {onBlurCapture?: (ev: Record<string, unknown>) => void}) => {
			blur = onBlurCapture;
			return <div />;
		});
	...
	test('should suppress onBlurCapture events when focus was suppressed', () => {
		...
		const Component = SpotlightContainerDecorator(({onBlurCapture, onFocusCapture}: {onBlurCapture?: (ev: Record<string, unknown>) => void; onFocusCapture?: (ev: Record<string, unknown>) => void}) => {
			blur = onBlurCapture;
			focus = onFocusCapture;
			return <div />;
		});
