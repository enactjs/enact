/*
 * Linear (Tab/Shift+Tab) traversal helpers for spotlight.
 *
 * These are pure helpers — they don't read or mutate any spotlight singleton state. State
 * (focus, restoration, container management) lives in spotlight.js. The per-keypress
 * `cache` is a `Map` passed in by the caller and discarded after the keypress; this avoids
 * module-level mutable state and makes lifetime explicit.
 */
...
function getLinearTargetsInContainer (containerId, cache) {
	...
	if (cache) {
		cache.set(containerId, targets);
	}

	return targets;
}

function resolveTargetToOpenPopupItem (target, currentPopupContainerId, isForward, cache) {
	...
	for (const popupContainerId of getOwnedSelfOnlyContainerIds(ownerNode, currentPopupContainerId)) {
		const popupTargets = getLinearTargetsInContainer(popupContainerId, cache);
 ...
 */
function findLinearTabExitTarget (focusedElement, selfOnlyContainerId, isForward, cache) {
	...
	const isRtl = isRtlDocument();
	const rootTargets = getLinearTargetsInContainer(rootContainerId, cache);
	...
	const pickExitTarget = (x, y, searchForward) => {
		...
		return candidate ?
			resolveTargetToOpenPopupItem(candidate, selfOnlyContainerId, isForward, cache) :
			null;
	};
...
/*
 * Linear Tab walk inside the search container. Returns true when focus moved.
 *
 * `deps` provides spotlight's focus operations: `getCurrent`, `restoreFocus`, `focusElement`.
 * `cache` is the per-keypress Map shared across all helper calls within one Tab event.
 */
function spotLinear (isForward, deps) {
	const {getCurrent, restoreFocus, focusElement, cache} = deps;
	...
	const linearTargets = getLinearTargetsInContainer(searchContainerId, cache);
	...
	if (nextIndex >= linearTargets.length) {
		if (scopeIsSelfOnly && isForward) {
			const exitTarget = findLinearTabExitTarget(currentFocusedElement, searchContainerId, true, cache);
			if (exitTarget) {
	...
	if (nextIndex < 0) {
		if (scopeIsSelfOnly && !isForward) {
			const exitTarget = findLinearTabExitTarget(currentFocusedElement, searchContainerId, false, cache);
			if (exitTarget) {
				return focusElement(exitTarget, getContainersForNode(exitTarget));
			}
		}
