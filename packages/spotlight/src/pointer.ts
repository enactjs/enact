 ...
 * @private
 */
const setPointerMode = (pointerMode: boolean): void => {
 ...
 * @private
 */
const hasPointerMoved = (x: number | null, y: number | null): boolean => (
 ...
 * @private
 */
const getLastPointerPosition = (): {x: number | null; y: number | null} => {
	return {
		x: _pointerX,
		y: _pointerY
	};
...
const hidePointerJob = new Job(function (callback?: () => void) {
...
const notifyPointerMove = (
	...
	if (updatePointerPosition(x, y)) {
		...
		return !priorPointerMode || !current || !current.contains(target as Node);
	}
 ...
 * @private
 */
const notifyKeyDown = (keyCode: number, callback?: () => void): boolean => {
