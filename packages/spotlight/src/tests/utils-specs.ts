...
type FocusableProps = Record<string, string | number | undefined>;

const focusable = (props?: FocusableProps) => {
	if (!props) {
		props = {};
	}

	const nodeObject: FocusableProps = {
		id: 'child',
		tabindex: 0
	};
...
describe('utils', () => {
	...
	describe('#isStandardFocusable', () => {
		beforeEach(() => {
			global.Element.prototype.getBoundingClientRect = jest.fn(() => toDOMRect({
				height: 100,
				width: 100,
				top: 0,
				left: 0
		...
		test('should return false if hidden', testScenario(
			...
			(root) => {
				const child = root.querySelector('#child')!;
				child.getBoundingClientRect = jest.fn(() => toDOMRect({
					height: 0,
					width: 0,
					top: 0,
					left: 0
