...
const focusTargetId = (containerId: string) => (getContainerFocusTarget(containerId) as Element).id;
...
describe('container', () => {
	...
	describe('#getContainersForNode', () => {
		test(
			...
			testScenario(
				...
				(root) => {
					...
					const actual = getContainersForNode(root.lastChild as Element);
		...
		test('should return all ancestor container ids', testScenario(
			scenarios.complexTree,
			(root) => {
				const childOfThird = queryEl(root, '#child-of-third');
				const expected = [
					rootContainerId,
					'first-container',
					'second-container',
	...
	describe('#getContainerFocusTarget', () => {
		...
		test(
			...
			testScenario(
				...
				(root) => {
					...
					setContainerLastFocusedElement(queryEl(root, '#lastFocused'), ['container']);
		...
		test(
			...
			testScenario(
				...
				() => {
					...
					const actual = (getContainerFocusTarget('first-container') as Element).parentElement!.dataset!.spotlightId;
		...
		test(
			...
			testScenario(
				...
				() => {
					...
					const actual = focusTargetId('second-container');
		...
		test('should cascade search into child containers with', testScenario(
			...
			(root) => {
				...
				setContainerLastFocusedElement(queryEl(root, '#lastChildFocused'), [rootContainerId, 'container', 'child']);
	...
	describe('#isNavigable', () => {
		...
		test(
			...
			testScenario(
				...
				(root) => {
					const expected = true;
					const actual = isNavigable(queryEl(root, '.other'), rootContainerId);
		...
		test(
			...
			testScenario(
				...
				(root) => {
					const expected = true;
					const actual = isNavigable(queryEl(root, '.spottable'), rootContainerId, true);
		...
		test('should return true for containers', testScenario(
			...
			(root) => {
				const expected = true;
				const actual = isNavigable(queryEl(root, `[${containerAttribute}]`), rootContainerId, true);
		...
		test(
			...
			testScenario(
				...
				(root) => {
					...
					const current = queryEl(root, '[data-spotlight-id="first-container"] .spottable');
					const target = queryEl(root, '.spottable');
		...
		test(
			...
			testScenario(
				...
				(root) => {
					...
					const current = queryEl(root, '[data-spotlight-id="first-container"] .spottable');
					const target = queryEl(root, '[data-spotlight-id="second-container"] .spottable');
		...
		test(
			...
			testScenario(
				...
				(root) => {
					...
					const current = queryEl(root, '[data-spotlight-id="second-container"] .spottable');
					const target = queryEl(root, '[data-spotlight-id="first-container"]');
	...
	describe('#getAllContainerIds', () => {
		...
		test('should not include inactive containers', testScenario(
			scenarios.onlyContainers,
			(root) => {
				const {containerId} = (queryEl(root, '[data-spotlight-id]') as HTMLElement).dataset;
				...
				const expected = -1;
				const actual = getAllContainerIds().indexOf(id);
		...
		test(
			...
			testScenario(
				...
				(root) => {
					...
					const actual = getNavigableContainersForNode(queryEl(root, '#secondContainerFirstSpottable'));
		...
		test(
			...
			testScenario(
				...
				(root) => {
					...
					const expected = queryEl(root, '#positionTargetContainer');
					const actual = getPositionTargetOnFocus(queryEl(root, '#firstChildSpottable'));
