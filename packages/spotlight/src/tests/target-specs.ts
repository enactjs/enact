...
const position = (top: number, left: number) => `position: absolute; top: ${top}px; left: ${left}px; height: 10px; width: 10px;`;

const positionedSpottable = (id: string, top: number, left: number) => {
	return spottable({
		id,
...
const safeTarget = <T, >(n: ContainerTarget | null | undefined, fn: (el: Element) => T): T | 'NOT FOUND' =>
	(n && typeof n !== 'string') ? fn(n) : 'NOT FOUND';

const queryEl = (root: ParentNode, selector: string): Element => root.querySelector(selector)!;
...
describe('target', () => {
	...
	describe('#getNavigableTarget', () => {
		test('should find spottable parent', testScenario(
			...
			(root) => {
				const other = queryEl(root, '.other');
		...
		test('should respect disabled containers', testScenario(
			...
			(root) => {
				...
				const other = queryEl(root, `[${containerAttribute}='third-container'] .spottable`);
	...
	describe('#getTargetByDirectionFromElement', () => {
		test('should find target within container by direction', testScenario(
			...
			(root) => {
				configureContainer('grid');
				const center = queryEl(root, '#middle-center');
				const topCenter = queryEl(root, '#top-center');
				const bottomCenter = queryEl(root, '#bottom-center');
				const middleLeft = queryEl(root, '#middle-left');
				const middleRight = queryEl(root, '#middle-right');

				center.getBoundingClientRect = () => toDOMRect({top: 10, left: 10, width: 10, height: 10});
				topCenter.getBoundingClientRect = () => toDOMRect({top: 0, left: 10, width: 10, height: 10});
				bottomCenter.getBoundingClientRect = () => toDOMRect({top: 20, left: 10, width: 10, height: 10});
				middleLeft.getBoundingClientRect = () => toDOMRect({top: 10, left: 0, width: 10, height: 10});
				middleRight.getBoundingClientRect = () => toDOMRect({top: 10, left: 20, width: 10, height: 10});
		...
		test('should find target within container from floating element', testScenario(
			...
			(root) => {
				...
				const gridContainer = queryEl(root, '[data-spotlight-id="grid"]');
				const overlap = queryEl(root, '#over-middle-center');
				const center = queryEl(root, '#middle-center');
				const topLeft = queryEl(root, '#top-left');
				const topCenter = queryEl(root, '#top-center');
				const topRight = queryEl(root, '#top-right');
				const middleLeft = queryEl(root, '#middle-left');
				const middleRight = queryEl(root, '#middle-right');
				const bottomLeft = queryEl(root, '#bottom-left');
				const bottomCenter = queryEl(root, '#bottom-center');
				const bottomRight = queryEl(root, '#bottom-right');

				// Mock getBoundingClientRect for all elements
				gridContainer.getBoundingClientRect = () => toDOMRect({top: 10, left: 10, width: 30, height: 30, right: 40, bottom: 40});
				topLeft.getBoundingClientRect = () => toDOMRect({top: 10, left: 10, width: 10, height: 10, right: 20, bottom: 20});
				topCenter.getBoundingClientRect = () => toDOMRect({top: 10, left: 20, width: 10, height: 10, right: 30, bottom: 20});
				topRight.getBoundingClientRect = () => toDOMRect({top: 10, left: 30, width: 10, height: 10, right: 40, bottom: 20});
				middleLeft.getBoundingClientRect = () => toDOMRect({top: 20, left: 10, width: 10, height: 10, right: 20, bottom: 30});
				center.getBoundingClientRect = () => toDOMRect({top: 20, left: 20, width: 10, height: 10, right: 30, bottom: 30});
				middleRight.getBoundingClientRect = () => toDOMRect({top: 20, left: 30, width: 10, height: 10, right: 40, bottom: 30});
				bottomLeft.getBoundingClientRect = () => toDOMRect({top: 30, left: 10, width: 10, height: 10, right: 20, bottom: 40});
				bottomCenter.getBoundingClientRect = () => toDOMRect({top: 30, left: 20, width: 10, height: 10, right: 30, bottom: 40});
				bottomRight.getBoundingClientRect = () => toDOMRect({top: 30, left: 30, width: 10, height: 10, right: 40, bottom: 40});

				// Overlap is a small element floating over middle-center
				overlap.getBoundingClientRect = () => toDOMRect({top: 22, left: 25, width: 1, height: 1, right: 26, bottom: 23});
		...
		test(
			...
			testScenario(
				...
				(root) => {
					...
					const overflowContainer = queryEl(root, '[data-spotlight-id="overflow-container"]');
					const element = queryEl(root, '#outside-overflow');
					const overflowAbove = queryEl(root, '#overflow-above');
					const overflowBelow = queryEl(root, '#overflow-below');
					const overflowWithin = queryEl(root, '#overflow-within');

					// Mock all bounding rects
					overflowContainer.getBoundingClientRect = () => toDOMRect({top: 110, left: 10, width: 30, height: 30, right: 40, bottom: 140});
					element.getBoundingClientRect = () => toDOMRect({top: 10, left: 10, width: 10, height: 10, right: 20, bottom: 20});
					overflowAbove.getBoundingClientRect = () => toDOMRect({top: 0, left: 10, width: 10, height: 10, right: 20, bottom: 110});
					overflowWithin.getBoundingClientRect = () => toDOMRect({top: 110, left: 10, width: 10, height: 10, right: 20, bottom: 120});
					overflowBelow.getBoundingClientRect = () => toDOMRect({top: 140, left: 10, width: 10, height: 10, right: 20, bottom: 150});
		...
		test(
			...
			testScenario(
				...
				(root) => {
					...
					const overflowContainer = queryEl(root, '[data-spotlight-id="overflow-container"]');
					const insideContainer = queryEl(root, '[data-spotlight-id="inside"]');
					const element = queryEl(root, '#outside-overflow');
					const inLargeContainer = queryEl(root, '#in-large-container');
					const belowLargeContainer = queryEl(root, '#below-large-container');

					// Mock all bounding rects
					overflowContainer.getBoundingClientRect = () => toDOMRect({top: 110, left: 10, width: 30, height: 30, right: 40, bottom: 140});
					insideContainer.getBoundingClientRect = () => toDOMRect({top: 100, left: 10, width: 10, height: 50, right: 20, bottom: 150});
					element.getBoundingClientRect = () => toDOMRect({top: 10, left: 10, width: 10, height: 10, right: 20, bottom: 20});
					inLargeContainer.getBoundingClientRect = () => toDOMRect({top: 110, left: 10, width: 10, height: 10, right: 20, bottom: 120});
					belowLargeContainer.getBoundingClientRect = () => toDOMRect({top: 150, left: 10, width: 10, height: 10, right: 20, bottom: 160});
		...
		test(
			...
			testScenario(
				...
				(root) => {
					...
					// Mock all bounding rects
					overflowContainer.getBoundingClientRect = () => toDOMRect({top: 110, left: 10, width: 30, height: 30, right: 40, bottom: 140});
					overflowAbove.getBoundingClientRect = () => toDOMRect({top: 100, left: 10, width: 10, height: 10, right: 20, bottom: 110});
					overflowWithin.getBoundingClientRect = () => toDOMRect({top: 110, left: 10, width: 10, height: 10, right: 20, bottom: 120});
					overflowBelow.getBoundingClientRect = () => toDOMRect({top: 140, left: 10, width: 10, height: 10, right: 20, bottom: 150});
		...
		test('should respect enterTo="default-element" containers', testScenario(
			...
			(root) => {
				...
				const gridContainer = queryEl(root, '[data-spotlight-id="grid"]');
				const beforeGrid = queryEl(root, '#before-grid');
				const afterGrid = queryEl(root, '#after-grid');
				const topLeft = queryEl(root, '#top-left');
				const topCenter = queryEl(root, '#top-center');
				const topRight = queryEl(root, '#top-right');
				const middleLeft = queryEl(root, '#middle-left');
				const middleCenter = queryEl(root, '#middle-center');
				const middleRight = queryEl(root, '#middle-right');
				const bottomLeft = queryEl(root, '#bottom-left');
				const bottomCenter = queryEl(root, '#bottom-center');
				const bottomRight = queryEl(root, '#bottom-right');
				...
				// Mock all bounding rects
				beforeGrid.getBoundingClientRect = () => toDOMRect({top: 0, left: 10, width: 30, height: 10, right: 40, bottom: 10});
				gridContainer.getBoundingClientRect = () => toDOMRect({top: 10, left: 10, width: 30, height: 30, right: 40, bottom: 40});
				afterGrid.getBoundingClientRect = () => toDOMRect({top: 40, left: 10, width: 30, height: 10, right: 40, bottom: 50});

				topLeft.getBoundingClientRect = () => toDOMRect({top: 10, left: 10, width: 10, height: 10, right: 20, bottom: 20});
				topCenter.getBoundingClientRect = () => toDOMRect({top: 10, left: 20, width: 10, height: 10, right: 30, bottom: 20});
				topRight.getBoundingClientRect = () => toDOMRect({top: 10, left: 30, width: 10, height: 10, right: 40, bottom: 20});
				middleLeft.getBoundingClientRect = () => toDOMRect({top: 20, left: 10, width: 10, height: 10, right: 20, bottom: 30});
				middleCenter.getBoundingClientRect = () => toDOMRect({top: 20, left: 20, width: 10, height: 10, right: 30, bottom: 30});
				middleRight.getBoundingClientRect = () => toDOMRect({top: 20, left: 30, width: 10, height: 10, right: 40, bottom: 30});
				bottomLeft.getBoundingClientRect = () => toDOMRect({top: 30, left: 10, width: 10, height: 10, right: 20, bottom: 40});
				bottomCenter.getBoundingClientRect = () => toDOMRect({top: 30, left: 20, width: 10, height: 10, right: 30, bottom: 40});
				bottomRight.getBoundingClientRect = () => toDOMRect({top: 30, left: 30, width: 10, height: 10, right: 40, bottom: 40});
		...
		test('should respect enterTo="last-focused" containers', testScenario(
			...
			(root) => {
				configureContainer('grid', {
					restrict: 'none',
					enterTo: 'last-focused',
					lastFocusedElement: queryEl(root, '#bottom-right')
				});
		...
		test('should ignore empty containers', testScenario(
			...
			(root) => {
				...
				const emptyContainer = queryEl(root, '[data-spotlight-id="empty-container"]');
				const above = queryEl(root, '#above');
				const below = queryEl(root, '#below');

				// Mock all bounding rects
				above.getBoundingClientRect = () => toDOMRect({top: 0, left: 10, width: 10, height: 10, right: 20, bottom: 10});
				emptyContainer.getBoundingClientRect = () => toDOMRect({top: 10, left: 10, width: 10, height: 10, right: 20, bottom: 20});
				below.getBoundingClientRect = () => toDOMRect({top: 30, left: 0, width: 10, height: 10, right: 10, bottom: 40});
		...
		test('should ignore overlapping empty containers', testScenario(
			...
			(root) => {
				...
				// Mock all bounding rects
				above.getBoundingClientRect = () => toDOMRect({top: 5, left: 10, width: 10, height: 10, right: 20, bottom: 15});
				emptyContainer.getBoundingClientRect = () => toDOMRect({top: 10, left: 10, width: 10, height: 10, right: 20, bottom: 20});
				below.getBoundingClientRect = () => toDOMRect({top: 30, left: 0, width: 10, height: 10, right: 10, bottom: 40});
	...
	describe('#getTargetByDirectionFromPosition', () => {
		test('should find target within container', testScenario(
			...
			(root) => {
				...
				middleCenter.getBoundingClientRect = () => toDOMRect({top: 10, left: 10, width: 10, height: 10});
				topCenter.getBoundingClientRect = () => toDOMRect({top: 0, left: 10, width: 10, height: 10});
				bottomCenter.getBoundingClientRect = () => toDOMRect({top: 20, left: 10, width: 10, height: 10});
				middleLeft.getBoundingClientRect = () => toDOMRect({top: 10, left: 0, width: 10, height: 10});
				middleRight.getBoundingClientRect = () => toDOMRect({top: 10, left: 20, width: 10, height: 10});

				const centerRect = queryEl(root, '#middle-center').getBoundingClientRect();
		...
		test(
			...
			testScenario(
				...
				(root) => {
					...
					const rect = queryEl(root, '#top-center').getBoundingClientRect();
		...
		test(
			...
			testScenario(
				...
				(root) => {
					...
					// Mock all bounding rects
					overflowContainer.getBoundingClientRect = () => toDOMRect({top: 50, left: 10, width: 30, height: 30, right: 40, bottom: 80});
					element.getBoundingClientRect = () => toDOMRect({top: 10, left: 10, width: 10, height: 10, right: 20, bottom: 20});
					overflowAbove.getBoundingClientRect = () => toDOMRect({top: 30, left: 10, width: 10, height: 10, right: 20, bottom: 40});
					overflowWithin.getBoundingClientRect = () => toDOMRect({top: 55, left: 10, width: 10, height: 10, right: 20, bottom: 65});
					overflowBelow.getBoundingClientRect = () => toDOMRect({top: 85, left: 10, width: 10, height: 10, right: 20, bottom: 95});
		...
		test(
			...
			testScenario(
				...
				(root) => {
					...
					// Mock all bounding rects
					overflowContainer.getBoundingClientRect = () => toDOMRect({top: 50, left: 10, width: 40, height: 40, right: 50, bottom: 90});
					insideContainer.getBoundingClientRect = () => toDOMRect({top: 40, left: 15, width: 30, height: 60, right: 45, bottom: 100});
					element.getBoundingClientRect = () => toDOMRect({top: 10, left: 10, width: 10, height: 10, right: 20, bottom: 20});
					inLargeContainer.getBoundingClientRect = () => toDOMRect({top: 60, left: 20, width: 10, height: 10, right: 30, bottom: 70});
					belowLargeContainer.getBoundingClientRect = () => toDOMRect({top: 110, left: 10, width: 10, height: 10, right: 20, bottom: 120});
	...
	describe('#directional visibility across containers', () => {
		test('should skip invisible target in another container and select a visible one', testScenario(
			...
			(root) => {
				...
				const overflowContainer = queryEl(root, `[${containerAttribute}='overflow-container']`);
				const outside = queryEl(root, '#outside-overflow');
				const above = queryEl(root, '#overflow-above');
				const within = queryEl(root, '#overflow-within');
				const below = queryEl(root, '#overflow-below');

				overflowContainer.getBoundingClientRect = () => toDOMRect({top: 0, left: 0, bottom: 30, right: 30, width: 30, height: 30});
				outside.getBoundingClientRect = () => toDOMRect({top: -20, left: 0, bottom: -10, right: 10, width: 10, height: 10});
				above.getBoundingClientRect = () => toDOMRect({top: -10, left: 0, bottom: -5, right: 10, width: 10, height: 10});
				within.getBoundingClientRect = () => toDOMRect({top: 0, left: 0, bottom: 10, right: 10, width: 10, height: 10});
				below.getBoundingClientRect = () => toDOMRect({top: 30, left: 0, bottom: 40, right: 10, width: 10, height: 10});
