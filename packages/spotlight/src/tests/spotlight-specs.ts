...
const mockFocus = (n: Element | null) => {
...
describe('Spotlight', () => {
	...
	describe('#initialize', () => {
		let addListenerSpy: jest.SpiedFunction<typeof window.addEventListener>;

		beforeEach(() => {
			addListenerSpy = jest.spyOn(window, 'addEventListener').mockImplementationOnce(() => {});
		});
		afterEach(() => {
	...
	describe('#terminate', () => {
		let removeListenerSpy: jest.SpiedFunction<typeof window.removeEventListener>;

		beforeEach(() => {
			removeListenerSpy = jest.spyOn(window, 'removeEventListener').mockImplementationOnce(() => {});
		});
		afterEach(() => {
