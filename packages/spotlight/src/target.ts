...
function getContainersToSearch (containerId?: string): string[] {
	let range: string[] = [];
	let addRange = function (id: string) {
...
function getTargetByContainer (containerId?: string, enterTo?: PreferredEnterTo): ContainerTarget | null {
	return getContainersToSearch(containerId)
		.reduce((next: ContainerTarget | null, id) => {
...
function getTargetBySelector (selector?: string | null): ContainerTarget | null {
	...
	if (selector.charAt(0) === '@') {
		if (selector.length === 1) {
			return getTargetByContainer();
		}

		return getTargetByContainer(selector.substr(1));
	}
	...
	if (next) {
		...
		if (isNavigable(next, last(nextContainerIds) as string, true)) {
			return next;
		}
	}
...
function isRestrictedContainer (containerId?: string): boolean {
	const config = containerId && getContainerConfig(containerId);
	return Boolean(config && (config.enterTo === 'last-focused' || config.enterTo === 'default-element'));
}

function filterRects (elementRects: Rect[], boundingRect?: Rect | null): Rect[] {
	if (!boundingRect) {
		return elementRects;
	}
	...
	return elementRects.filter(rect => {
		...
		if (topUpdate || bottomUpdate || leftUpdate || rightUpdate) {
			const updated: Rect = {...rect, center: {...rect.center}};
...
function getContainerContainingRect (elementRects: Rect[], elementRect: Rect): string | false {
	...
	if (overlapping.length) {
		return (overlapping[0].element as HTMLElement).dataset.spotlightId as string;
	}

	return false;
}

function getOverflowContainerRect (containerId: string): Rect | null | undefined {
...
function getTargetInContainerByDirectionFromPosition (
		direction: Direction,
		containerId: string,
		positionRect: Rect,
		elementContainerIds: string[],
	...
	while (elementRects.length > 0) {
		...
		if (overlappingContainerId) {
			...
			if (!next) {
				...
				elementRects = elementRects.filter(rect => {
					return (rect.element as HTMLElement).dataset.spotlightId !== overlappingContainerId;
				});
				continue;
			}
		...
		next = navigate(
			...
			direction,
			elementRects,
			getContainerConfig(containerId) ?? null
		);

		// if we match a container, recurse into it
		if (next && isContainer(next)) {
			const nextContainerId = (next as HTMLElement).dataset.spotlightId as string;
			...
			if (isRestrictedContainer(nextCandidateContainerId) || isElementVisibleInContainer(next, nextCandidateContainerId) || isElementVisibleInViewport(next, nextCandidateContainerId)) {
				...
				next = navigate(
					...
					elementRects.filter((rect) => {
						...
						return isElementVisibleInContainer(rect.element as Element, elementContainerId) || isElementVisibleInViewport(rect.element as Element, elementContainerId);
					}),
					getContainerConfig(containerId) ?? null
				);
			}
...
function getTargetInContainerByDirectionFromElement (
		direction: Direction,
		containerId: string,
		element: Element,
		elementRect: Rect,
	...
	if (previous && elements.indexOf(previous as Element) !== -1) {
		return previous;
	}
	...
	if ((element as HTMLElement).dataset.spotlightOverflow === 'ignore') {
		boundingRect = getOverflowContainerRect(containerId) || boundingRect;
	}

	let elementRects = filterRects(getRects(elements), boundingRect);

	let next: ContainerTarget | null = null;
	...
	while (elementRects.length > 0) {
		...
		let partitionRect: Rect = elementRect;
		if (partitionContainer) {
			partitionRect = getContainerRect(partitionContainer) as Rect;
		}
		...
		const nextCandidateContainerId = getContainersForNode(next as Element | null).pop();
		// check if we want to navigate to another container
		if (next && containerId !== nextCandidateContainerId) {
			// Don't apply visibility check when entering a restricted container. The container's own enterTo logic (last-focused, default-element) handles entry
			// OR verify if the element from another container is visible
			if (isRestrictedContainer(nextCandidateContainerId) || isElementVisibleInContainer(next as Element, nextCandidateContainerId) || isElementVisibleInViewport(next as Element, nextCandidateContainerId)) {
				return next;
			} else {
				// otherwise, try to navigate to an element that is visible in its container
				next = navigate(
...
function getTargetByDirectionFromElement (direction: Direction, element: Element, getIntersectRectOfElem: boolean = false): ContainerTarget | null {
	...
	const next = getNavigableContainersForNode(element)
		.reduceRight((result: ContainerTarget | boolean | null, containerId, index, elementContainerIds) => {
			result = result || getTargetInContainerByDirectionFromElement(
				direction,
				containerId,
				element,
				getIntersectRectOfElem ? getIntersectionRect(getContainerNode(elementContainerId as string) as Element, element) : getRect(element),
				elementContainerIds
			);
	...
	if (next !== element) {
		return next as ContainerTarget | null;
	} else if (elementContainerId !== rootContainerId && getContainerConfig(elementContainerId as string)?.overflow && !getIntersectRectOfElem) {
...
function getTargetByDirectionFromPosition (direction: Direction, position: Position, containerId: string): Element | null {
	const pointerRect = getPointRect(position);

	return getNavigableContainersForNode(getContainerNode(containerId))
		.reduceRight((result: Element | null, id, index, elementContainerIds) => {
			return result ||
 ...
 * @private
 */
function getLeaveForTarget (containerId: string, direction: Direction): ContainerTarget | boolean | null {
	...
	if (config) {
		...
		const nextContainerIds = getContainersForNode(target || null);
		if (isNavigable(target || null, last(nextContainerIds) as string)) {
...
function getNavigableTarget (target: Document | Element | null | undefined): Element | null {
	if (target === document) return null;

	let parent: Document | Element | null;
	while (target && (isContainer(target) || !isFocusable(target as Element))) {
		parent = (target as Element).parentNode as Document | Element | null;
		target = parent === document ? null : parent;
	}
	return target as Element | null;
}

function isElementVisibleInContainer (element: Element, containerId?: string): boolean {
	const {
		top: containerTop,
		right: containerRight,
		bottom: containerBottom,
		left: containerLeft
	} = getContainerRect(containerId as string) as Rect;
	const {
		top: elementTop,
		right: elementRight,
...
function isElementVisibleInViewport (element: Element, containerId?: string): boolean {
...
const getOffsetDistanceToTargetFromPosition = (
		distance: number,
		direction: Direction,
		{x, y}: Position,
...
const getDistanceToTargetFromPosition = (direction: Direction, position: Position, elementRect: Rect): number => {
	const {x, y} = position;
	let distance = 0;
...
const getNearestTargetsInContainerFromPosition = (position: Position, containerId: string): DirectionalTarget[] => {
	return (['up', 'left', 'right', 'down'] as Direction[]).reduce((result: DirectionalTarget[], direction) => {
...
const getNearestTargetInContainerFromPosition = (position: Position, containerId: string): Element | undefined => {
...
const getNearestTargetFromPosition = (position: Position, containerId: string): Element | null | undefined => (
