...
import type {SpottableConfig} from '../types/SpottableConfig';
import type {SpottableProps} from '../types/SpottableProps';
...
const callContext = (name: string) => (ev: any, props: any, context: any) => context[name](ev, props);
const spotHandlers = {
	onKeyDown: handle(
		forwardKeyDown,
 ...
 */
const defaultConfig: SpottableConfig = {
	/**
 ...
 */
const Spottable = hoc(defaultConfig, (config: SpottableConfig, Wrapped) => {
	const {emulateMouse} = config;
	const WrappedWithRef = WithRef(Wrapped);

	function SpottableBase (props: SpottableProps) {
	...
	function Spottable (props: SpottableProps) {
