...
const wrapInCells = function ({cellProps = [], children = []}) {
	...
	if (children instanceof Array) {
		// `children` is empty, bail
		if (!children.length) return null;
	} else {
		// `children` isnt array, coerse it into an array
		children = [children];
	}

	const wrappedChildren = [];

	// console.groupCollapsed('wrapInCells');
	React.Children.forEach(children, (child, index) => {
		// console.log('child:', child);
		if (child && child.type !== Cell) {
			wrappedChildren.push(<Cell key={`autoGeneratedCell-${index}`} {...cellProps[index]}>{child}</Cell>);
		} else if (!child) {
			// null, do nothing
		} else {
