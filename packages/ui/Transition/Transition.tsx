...
function Transition ({
	...
	const [state, setState] = useState<{initialHeight: number | null; initialWidth: number | null; renderState: number}>(() => ({
		initialHeight: null,
		initialWidth: null,
	...
	const handleTransitionEnd = useCallback((ev: TransitionEvent) => {
		...
		if (ev.target === childNodeRef.current) {
			...
			forward(eventName, {type: eventName, currentTarget: ev.currentTarget as EventTarget}, propsRef.current);
		}
	}, []);
