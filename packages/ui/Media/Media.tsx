 ...
 * @public
 */
const getKeyFromSource = (source: any = '') => {
	if (isValidElement(source)) {
		return Children.toArray(source)
			.filter(s => !!s)
			.map((s: any) => s.props.src)
			.join('+');
	}
...
class Media extends ReactComponent<Record<string, any>, Record<string, any>> {
	...
	constructor (props: Record<string, any>) {
		...
		for (let eventName in props.customMediaEventsMap) {
			...
			this.handledCustomMediaForwards[eventName] = (ev: any) => forwardEvent(ev, this.props);
		}
	}
	...
	handleEvent = (ev: any) => {
		...
		const fwd = this.handledMediaForwards[(handledMediaEventsMap as Record<string, string>)[ev.type]];
		if (fwd) {
			fwd(ev, this.props);
		}
	};
	...
	render () {
		const {customMediaEventsMap, mediaComponent: Component, source, ...rest}: Record<string, any> = this.props;
