...
describe('useAnnounce', () => {
	...
	function Base ({children, announce}: {children?: ReactNode; announce?: (...args: any[]) => any}) {
		useEffect(() => {
	...
	test.skip('should set the value passed to announce into the ARIA role="alert" node', () => {
		...
		(component as any).find(Base).invoke('announce')(text);
		...
		const actual = (component as any).find({role: 'alert'}).instance().getAttribute('aria-label');
