...
const RouterBase = class extends ReactComponent<Record<string, any>, Record<string, any>> {
	 ...
	 */
	createRoutes (children: any, routes: Record<string, any>): Record<string, any> {
	...
	createChildren () {
		...
		const childrenElements = segments.map((segment: string, index: number) => {
	...
	render () {
		const {component: Component, componentRef, ...rest}: Record<string, any> = this.props;
