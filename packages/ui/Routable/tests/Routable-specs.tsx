...
describe('Routable', () => {
	test('should render nothing for a partially valid path', async () => {
		...
		const Views = Routable({navigate: 'onNavigate'}, ({children}: {children?: any}) => <div>{children}</div>);

		function Sample (props: Record<string, any>) {
			const [path, nav] = useState('/app');
			const handleNavigate = useCallback((ev: any) => nav(ev.path), [nav]);
...
describe('Route', () => {
	let consoleWarnMock: any = null;
	let consoleErrorMock: any = null;
