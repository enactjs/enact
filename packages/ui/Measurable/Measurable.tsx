...
const useMeasurable = () => {
	const [measurement, setMeasurement] = useState<DOMRect | undefined>();
				...
				let mutationObserver: MutationObserver | null = new MutationObserver((mutationsList) => {
					for (let mutation of mutationsList) {
						if (mutation.type === 'childList') {
							setMeasurement((mutation.target as Element).getBoundingClientRect());
...
const Measurable = hoc(defaultConfig, (configHoc, Wrapped) => {
	return function MeasurableHoc (props: Record<string, any>) {
