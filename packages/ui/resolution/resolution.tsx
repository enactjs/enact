...
interface ResolutionConfig {
	fontSizeHandling: 'normal' | 'scale';
	orientationHandling: 'normal' | 'scale';
	linearScaling: {
		active: boolean;
		type: string;
	};
...
let baseScreen: ScreenType | undefined,
	orientation: 'landscape' | 'portrait' | undefined,
	riRatio: number | undefined,
	...
	screenTypeObject: ScreenType | undefined,
 ...
 * @public
 */
const unitToPixelFactors: Record<string, number> = {
	'rem': 12,
	'in': 96
};
 ...
 * @private
 */
const configDefaults: ResolutionConfig = {
 ...
 * @private
 */
function getLinearSize (): number {
	const isCurrentScreen = config.linearScaling.type === linearScalingType.currentScreen;
	const reportScreen = (isCurrentScreen ? screenTypeObject : baseScreen) as ScreenType;
	const maxSize = (screenTypes.at(-1) as ScreenType).pxPerRem;
 ...
 * @private
 */
const getShouldScale = (scrObj: ScreenType | undefined = screenTypeObject): boolean => {
 ...
 * @private
 */
const getScaleFactor = (type: string | undefined = screenType): {shouldScale: boolean; factor: number} => {
...
const getClosestResolutionType = (resolution: Bounds, types: ScreenType[]): string => {
	...
	const {name} = types.reduce((prev: ScreenType, curr: ScreenType) => {
 ...
 * @private
 */
const updateWorkspaceBounds = (measurementNode: any): void => {
	if (measurementNode) {
 ...
 * @private
 */
function getScreenTypeObject (type?: string): ScreenType {
 ...
 * @public
 */
function defineScreenTypes (types?: ScreenType[]): void {
 ...
 * @public
 */
function getScreenType (rez?: Bounds): string {
 ...
 * @public
 */
function calculateFontSize (type?: string): string {
 ...
 * @private
 */
function updateBaseFontSize (size: string): void {
 ...
 * @public
 */
function getResolutionClasses (type: string | undefined = screenType): string {
 ...
 */
function getRiRatio (type: string | undefined = screenType): number {
	if (type && baseScreen) {
 ...
 */
function getUnitToPixelFactors (type: string | undefined = screenType): number {
 ...
 * @public
 */
function getAspectRatio (type?: string): number {
	const scrObj = getScreenTypeObject(type);
 ...
 * @public
 */
function getAspectRatioName (type?: string): string {
	const scrObj = getScreenTypeObject(type);
 ...
 * @public
 */
function unit (pixels: string | number, toUnit: string): string | undefined {
 ...
 * @public
 */
const scaleToRem = (pixels: number) => unit(scale(pixels), 'rem');
 ...
 * @public
 */
function selectSrc (src: string | Record<string, string>): string {
	if (typeof src != 'string' && src) {
		...
		for (let i = types.length - 1; i >= 0; i--) {
			...
			// return exact match
			if (screenType === t && (src as Record<string, string>)[t]) return (src as Record<string, string>)[t];
		}

		const srcResolutions = types.filter((type) => type.name in (src as Record<string, string>));
		...
		if (srcResolutions.length && currentScreenTypeResolution) {
			...
			newSrc = (src as Record<string, string>)[closestType];
		}
 ...
 * @public
 */
function init (args: {measurementNode?: any} = {}): void {
