...
describe('Cancelable', () => {
	// Suite-wide setup
	const Component = ({children, className, onKeyUp, ...rest}: Record<string, any>) => (
	...
	test('should call onCancel when additional cancel handlers pass', () => {
		const customCancelHandler = (ev: any) => ev.keyCode === 461;
	...
	describe('modal instances', () => {
		const customEventHandler = (ev: any) => {
			return ev.keyIdentifier === '27';
		};

		const makeKeyboardEvent = (keyCode: number) => {
			return new window.KeyboardEvent('keyup', {keyCode, code: String(keyCode), bubbles: true} as any);
		};
