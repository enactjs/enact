...
const Cancelable = hoc(defaultConfig, (config, Wrapped) => {
	...
	const dispatchCancelToConfig = function (ev: any, props: Record<string, any>) {
	...
	return class extends ReactComponent<Record<string, any>, Record<string, any>> {
		...
		renderWrapped (props: Record<string, any>) {
			return (
		...
		renderUnwrapped (props: Record<string, any>) {
			return (
				<Wrapped {...props} onKeyUp={this.handleKeyUp} />
			);
		}

		renderModal (props: Record<string, any>) {
			return (
				<Wrapped {...props} />
			);
		}
