	...
	return class ScrollableChildProviderCore extends Component {
		...
		calculateMetrics (props) {
			const
				...
				node = this.virtualRepeaterRef.containerRef;
			...
			if (this.virtualRepeaterRef) {
				// eslint-disable-next-line react/no-direct-mutation-state
				this.virtualRepeaterRef.state.firstIndex = 0;
				// eslint-disable-next-line react/no-direct-mutation-state
				this.virtualRepeaterRef.state.numOfItems = 0;
			}
		}
		...
		updateStatesAndBounds = (props) => {
			...
			this.virtualRepeaterRef.setState({firstIndex: newFirstIndex, numOfItems});
		}
		...
		setContainerSize = () => {
			if (this.virtualRepeaterRef &&  this.virtualRepeaterRef.contentRef) {
				this.virtualRepeaterRef.contentRef.style.width = this.scrollBounds.scrollWidth + 'px';
				this.virtualRepeaterRef.contentRef.style.height = this.scrollBounds.scrollHeight + 'px';
			}
		}
		...
		scrollToPosition (x, y) {
			if (this.virtualRepeaterRef && this.virtualRepeaterRef.containerRef) {
				this.virtualRepeaterRef.containerRef.scrollTo(
		...
		setScrollPosition (x, y, dirX, dirY) {
			if (this.virtualRepeaterRef && this.virtualRepeaterRef.contentRef) {
				this.virtualRepeaterRef.contentRef.style.transform = `translate3d(${this.props.rtl ? x : -x}px, -${y}px, 0)`;
		...
		didScroll (x, y, dirX, dirY) {
			const
				{dataSize} = this.props,
				{firstIndex} = this.virtualRepeaterRef.state,
			...
			if (firstIndex !== newFirstIndex) {
				this.virtualRepeaterRef.setState({firstIndex: newFirstIndex});
			}
		}
		...
		getItemNode = (index) => (this.virtualRepeaterRef.getItemNode(index))

		setProvider = (ref) => {
			this.virtualRepeaterRef = ref;
		}
