...
import {createContext, type ReactNode, useCallback, useMemo, useEffect, useRef} from 'react';
...
const useMarqueeController = (props: Record<string, any>) => {
	 ...
	 * @returns	{undefined}
	 */
	const markAll = useCallback((state: number) => {
	...
	const allInactive = useCallback(() => {
		const activeOrReady = mutableRef.current.controlled.reduce((res: boolean, component) => {
	...
	const anyRunning = useCallback(() => {
		return mutableRef.current.controlled.reduce((res: boolean, component) => {
			return res || (component.state === STATE.active);
		}, false);
	}, []);

	const doCancel = useCallback((retryStartingAnimation?: boolean) => {
	 ...
	 * @returns	{undefined}
	 */
	const handleCancel = useCallback((retryStartingAnimation?: boolean) => {
		if (anyRunning()) {
	...
	const provideMarqueeControllerContext = useCallback((children: ReactNode) => {
		return (
