...
const
	...
	Util_getNumberValue = (index) => index | 0,
	Util_nop = () => {},
...
function onKeyUpAndDown () {
	...
	function onAcceleratedKeyDown ({isWrapped, keyCode, nextIndex, repeat, target}) {
		...
		const index = Util_getNumberValue(target.dataset.index);
	...
	function onKeyDown (ev) {
		...
		if (direction) {
			...
			if (SpotlightAccelerator.processKey(ev, Util_nop)) {
				ev.stopPropagation();
			} else {
				const {repeat} = ev;
				...
				const index = !isScrollButton ? Util_getNumberValue(targetIndex) : -1;
					...
					if (nextIndex >= 0) {
						...
						if (repeat && isLeaving) {
							...
							if (typeof nextTargetIndex === 'string') {
								onAcceleratedKeyDown({keyCode, nextIndex: Util_getNumberValue(nextTargetIndex), repeat, target});
							}
						}
...
function Placeholder () {
	...
	function handlePlaceholderFocus (ev) {
		...
		if (placeholder) {
			...
			if (index) {
				variables.current.preservedIndex = Util_getNumberValue(index);
				variables.current.restoreLastFocused = true;
			}
		}
	}
...
function Focus () {
	...
	function calculatePositionOnFocus ({item, scrollPosition = variables.current.uiRefCurrent.scrollPosition}) {
		const
			...
			focusedIndex = Util_getNumberValue(item.getAttribute(dataIndexAttribute));
	...
	function setLastFocusedNode (node) {
		variables.current.lastFocusedIndex = node.dataset && Util_getNumberValue(node.dataset.index);
	}
}
