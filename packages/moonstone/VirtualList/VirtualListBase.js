...
const VirtualListBaseFactory = (type) => {
	...
	const VirtualListCore = (props, ref) => {
		...
		// useEffect
		usePreventScroll(uiRefCurrent ? uiRefCurrent.containerRef.current : null);

		useKeyDown(variables.current, props);
		...
		} = useSpotlight(variables.current, props);

		const {
			shouldPreventOverscrollEffect
		} = useOverscrollEffect(variables.current, props);

		const {
			calculatePositionOnFocus
		} = useScrollable(variables.current, props);

		const initUiRef = useCallback(
			(ref) => {
				if (ref) {
			...
			[props.initUiChildRef]
		);

		const initItemRef = useCallback(
			(ref, index) => {
				if (ref) {
					if (type === JS) {
						focusByIndex(index);
		...
		const getComponentProps = useCallback(
			(index, nodeIndexToBeFocused) => {

				return (index === nodeIndexToBeFocused) ? {ref: (ref) => initItemRef(ref, index)} : {};
			},
			[variables.current.nodeIndexToBeFocused, initItemRef]
		);
		...
		useImperativeHandle(
			...
			() => ({
				getScrollBounds () {
					return variables.current ? variables.current.uiRefCurrent.getScrollBounds() : null;
				}
			})
		);
		*/
		...
		return (
			<UiBase
				{...rest}
				getComponentProps={(index) => getComponentProps(index, variables.current.nodeIndexToBeFocused)}
