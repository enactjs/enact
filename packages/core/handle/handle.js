...
const oneOf = handle.oneOf = function (...handlers) {
	return handle.call(this, function (...args) {
		for (let i = 0; i < handlers.length; i++) {
			if (handlers[i][0].apply(this, args)) {
				return handlers[i][1].apply(this, args);
			}
		}
 ...
 * @public
 */
const forward = handle.forward = curry3(named((name, ev, props) => {
 ...
 * @private
 */
const forwardWithPrevent = handle.forwardWithPrevent = curry3(named((name, ev, props) => {
	let prevented = false;
