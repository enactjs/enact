...
>* `Node` refers to one or more renderable instance: `<div>HTML Element</div>`, `<Button>React Component</Button>`, or `'string'`;
>* `Element` is a single instance of React.Element as returned by a component's render method: `<div>HTML Element</div>` or `<Button>React Component</Button>`
>* `Component` is function or class that returns a React.Element (e.g.: `'div'` or `Button`). This will typically be used to create dynamic JSX; something like: `render ({YourProp}) => { return (<YourProp />); }`
